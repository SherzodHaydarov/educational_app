...
def payment_qrcode(payment_id: int, db: Session = Depends(get_db)):
    ...
    if not payment:
        raise HTTPException(status_code=404, detail="Payment not found")
    image_b64 = payment_service.generate_qr_base64(payment.qr_payload)
    return PaymentQRCode(payment_id=payment.id, qr_image_base64=image_b64, payload=payment.qr_payload)
